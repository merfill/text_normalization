...
class TextGenerator:
    ...
    def beam_search_sampling(self, sess, char_input, beam_size=5):
        input_state = sess.run(self._initial_state, feed_dict={self._char_input: char_input})

        def gen_next(beams):
            batch_size = beams.shape[0]
            word_input = np.zeros([batch_size, self._out_seq_len, self._word_embedding_size])
            for i in range(beams.shape[1]):
                state = beams[:,i][1]
                for j in batch_size:
                    word_id = beams[:,i][0][:-1][j]
                    word_input[j,0] = self._word_embeddings[word_id]
                ...
                candidates = np.argsort(prediction, axis=1)[:,-beam_size:]
        ...
        state, prediction = sess.run([self._final_state, self._prediction],
                                     feed_dict={self._word_input: word_input, self._initial_state: input_state})

        batch_size = prediction.shape[0]
        beams = np.array([[([self._start_word_id], state, 0., False)] for _ in batch_size])
